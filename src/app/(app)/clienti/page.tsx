  ...
  return (
    <div className="space-y-6">
      <header className="flex flex-wrap items-end justify-between gap-4">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight text-concrete-900">Clienți</h1>
        ...
        <div className="card overflow-x-auto">
          <table className="w-full min-w-[640px]">
            ...
            <tbody className="divide-y divide-concrete-200">
              {clients.map((client) => (
                <tr key={client.id} className="hover:bg-concrete-50">
                  <td className="table-cell font-medium">{client.name}</td>
                  <td className="table-cell text-concrete-500">{client.cui ?? "—"}</td>
                  <td className="table-cell text-concrete-500">
                    {[client.contact_person, client.phone].filter(Boolean).join(" · ") || "—"}
                  </td>
                  <td className="table-cell text-concrete-500">{client.city ?? "—"}</td>
